...
import bg from "../assets/robot-scene.jpg"
...
function SignIn() {
  ...
  const handleSignIn = async (e) => {
    ...
    try {
        ...
        setLoading(false);
        toast.success(result.data.message || "Sign-in successful! Redirecting...");

        // Redirect based on assistant image status
        if (result.data.hasAssistantImage) {
            navigate("/");
        } else {
            navigate("/customize");
        }
  ...
  return (
    <div className='w-full h-[100vh] bg-cover flex justify-center items-center' style={{ backgroundImage: `url(${bg})` }} >
      <Toaster />
      <form className='w-[90%] h-[600px] max-w-[500px] bg-[rgba(0,0,0,0.2)] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-[20px] px-[20px]' onSubmit={handleSignIn}>
